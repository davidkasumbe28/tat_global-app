...
import { readTransactionUser } from "@/app/(server)/services/transaction.service";
...
export async function GET(
  ...
  try {
    return await checkAuth(request, async () => {
      ...
      const result = await readTransactionUser(parseInt(id));
    ...
    console.error("GET transaction user API error : ", error);
