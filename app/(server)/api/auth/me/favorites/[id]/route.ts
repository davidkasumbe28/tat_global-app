...
export async function GET(
  ...
  try {
    return await checkAuth(request, async () => {
      ...
      const result = await readFavorite(parseInt(id));

      if (!result.success) {
        return NextResponse.json({ error: result.error }, { status: 400 });
      }

      return NextResponse.json({ data: result.favorite }, { status: 200 });
    });
  } catch (error) {
    console.error("GET favorite API error:", error);
...
export async function DELETE(
  ...
  try {
    return await checkAuth(request, async () => {
      ...
      const result = await deleteFavorite(userId, parseInt(id));
