...
export async function POST(request: NextRequest) {
  try {
    const { deliveryPersonId, userId, orderId } = await request.json();
    ...
    return NextResponse.json({ data: res.delivery }, { status: 200 });
  } catch (error) {
    console.error("POST delivery API error:", error);
    return NextResponse.json({ error: logs.error.server }, { status: 500 });
  }
}
