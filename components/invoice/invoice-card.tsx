...
export default function AccountInvoiceCard({
  item,
  emuted = true,
}: AccountInvoiceCardProps) {
  return (
    <div
      key={item.id}
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div className="flex flex-row items-center justify-between gap-10">
          <div>
            <Skeleton emuted={emuted}>
              <p className="font-semibold text-lg">{item.invoiceNumber}</p>
            </Skeleton>
            <Skeleton emuted={emuted}>
              <p className={cn("text-sm", !emuted && "text-gray-500")}>
                {formatDate(new Date(item.issueDate || new Date()))}
              </p>
            </Skeleton>
          </div>
          ...
          <div className="text-right">
            ...
            <Skeleton emuted={emuted}>
              <p className={cn("text-sm", !emuted && "text-gray-500")}>
                {item?.order?.cart?._count.cartItems}{" "}
                {(item?.order?.cart?._count.cartItems || 0) > 1
                  ? "articles"
                  : "article"}
              </p>
            </Skeleton>
        ...
        <div className="flex justify-end items-center gap-3">
              ...
              className={cn(
                ...
                !emuted &&
                  (item?.status === StatusInvoice.PAID
                    ...
                    : item?.status === StatusInvoice.DRAFT
                      ? "bg-blue-100 text-blue-700"
                      : item?.status === StatusInvoice.CANCELLED
          ...
          <Link href={emuted ? "#" : APP.private.invoices + "/" + item.id}>
            <Button
